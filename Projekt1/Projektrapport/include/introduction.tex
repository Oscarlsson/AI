...
One part of supervised learning is the field Text classification which is to
...
\\\\
Build a feature vector which is an n-dimensional vector of numerical features
...
Cross Validation is used to test classification algorithms on sets of data that the classifiers hasn't seen, in order to get an accurate evaluation of a hypothesis. \ref{crossvalid_ai}\\\\
K-fold cross validation means that the data is split into k roughly equal sizes and then k rounds of training is performed with 1/k as test data and the rest as training. The average score will be better than one single score. K-values between 5 and 10 are popular and enough to get accurate results \ref{crossvalid_ai}.
